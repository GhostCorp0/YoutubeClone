...
final allChannelsProvider = Provider((ref) async {
  ...
  List<UserModel> users = usersMap.docs.map((user) => UserModel.fromJson(user.data())).toList();
  return users;
});

final allVideosProvider = Provider((ref) async {
  final videosMap = await FirebaseFirestore.instance.collection("videos").get();
  List<VideoModel> videos = videosMap.docs.map((video) => VideoModel.fromMap(video.data())).toList();
  return videos;
});
